...
- Dual-mode scope: `docstats/scope.py` defines the `Scope` frozen dataclass + `scope_sql_clause()` helper + `ScopeRequired` exception. `routes._common.get_scope` is the FastAPI dep; silently falls back to solo mode if `active_org_id` is stale. Every Patient/Referral row from Phase 1+ carries exactly one of `scope_user_id` or `scope_organization_id` — enforced by CHECK constraint on the DB and scope filter on every read/write
